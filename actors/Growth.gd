...
onready var _foliage: Node2D = $"%Foliage"
...
func _ready():
  ...
  var _num_foliage: int = (randi() % 6) + 1
  for _i in range(_num_foliage):
    var _new_foliage: Sprite = Sprite.new()
    var _new_position: Vector2 = global_position + (Vector2(rand_range(-1, 1), rand_range(-1, 1)).normalized() * (_asteroid_data.size * rand_range(5, 15)))
    ...
    _new_foliage.set_meta("scale_offset", rand_range(0, 1000))
